    ...
    filteredTransactions.forEach((transaction) => {
        ...
        row.innerHTML = `
            <td class="transaction-id">
                ...
                <span>
                    ${escapeHtml(transaction.bookingId)} · ${escapeHtml(transaction.date)}
                </span>
            </td>

            <td class="person-cell">
                <strong>${escapeHtml(transaction.customer)}</strong>
                <span>${escapeHtml(transaction.customerEmail)}</span>
            </td>

            <td class="person-cell">
                <strong>${escapeHtml(transaction.provider)}</strong>
                <span>${escapeHtml(transaction.providerEmail)}</span>
            </td>
...
function renderPendingPayouts() {
    const pendingTransactions = transactions.filter(
        (transaction) =>
            ["verified", "partially-refunded"].includes(transaction.paymentStatus) &&
            transaction.payoutStatus === "pending" && !transaction.payoutBatchId &&
            !refunds.some((refund) => refund.transaction_id === transaction.databaseId &&
    ...
    if (pendingTransactions.length === 0) {
        pendingPayoutList.innerHTML = `
            <div class="empty-state">
                ...
                <p>No unbatched payouts are currently eligible for settlement.</p>
            </div>
        `;
    ...
    pendingProviders.forEach((provider) => {
        ...
        payoutItem.innerHTML = `
            <div class="provider-avatar">
                ${escapeHtml(createInitials(provider.provider))}
            </div>
...
function openPayoutModal(providerId) {
    const providerTransactions = transactions.filter(
        (item) => item.providerId === providerId && ["verified", "partially-refunded"].includes(item.paymentStatus) &&
            item.payoutStatus === "pending" && !item.payoutBatchId &&
            !refunds.some((refund) => refund.transaction_id === item.databaseId && ["REQUESTED", "APPROVED"].includes(refund.status))
    );
...
function renderSettlements() {
    ...
    settlements.forEach((settlement) => {
        ...
        row.innerHTML = `
            <td><strong>${escapeHtml(settlement.settlement_reference)}</strong></td>
            <td>${escapeHtml(settlement.provider_name || settlement.provider_email || `Provider #${settlement.provider_id}`)}</td>
            <td>${escapeHtml(settlement.period_start)} – ${escapeHtml(settlement.period_end)}</td>
...
function mapTransaction(transaction) {
    ...
    return {
        ...
        providerShare: Number(transaction.payout_amount ?? transaction.provider_payable ?? 0),
        payoutBatchId: transaction.payout_batch_id,
