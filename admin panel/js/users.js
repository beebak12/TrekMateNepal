...
async function providerDetails(id) {
    ...
    try {
        ...
        $("providerEarnings").textContent = money(transactions.filter((item) => item.payout_status === "PAID")
            .reduce((sum, item) => sum + Number(item.payout_amount ?? item.provider_payable ?? 0), 0));
